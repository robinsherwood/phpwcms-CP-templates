...
(function($) {
	...
	function GridAccordion(instance, options) {
		...
		var accordion = $(instance),
			...
			panelProps = ['captionFadeDuration', 'captionWidth', 'captionHeight', 'captionTop', 'captionLeft', 'linkTarget', 'alignType'],
		...
		*/
		function init() {
			...
			accordion.addClass('grid-accordion')
		...
		*/
		function createPanel(index) {
			...
			$('<img/>').load(function() {
							...
							switch (panels[index].properties.alignType) {
								case 'leftTop':
							...
							panels[index].width	= $(this).attr('width') || $(this).prop('width');
							panels[index].height = $(this).attr('height') || $(this).prop('height');
